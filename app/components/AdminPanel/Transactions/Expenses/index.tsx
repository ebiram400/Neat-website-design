...
export default function ExpensesIndex({ projectId } : { projectId: string }) {

    const consumptionMaterialInfos:consumptionMaterialInfo[] = [
        { id:0, description:"کرایه وانت سنگ", amount:"10000000", date:"1404-06-15" },
        { id:1, description:"اجاره هیلتی", amount:"50000000", date:"1404-06-14" },
        { id:2, description:"بیمه کارگاه", amount:"18800000", date:"1404-06-11" },
        { id:3, description:"قند و چای", amount:"520000", date:"1404-06-11" }
    ]
    ...
    const handleEdit = (transactionId:number)=>{
        route.push(`/dashboard/projects/${projectId}/expenses/new&edit?transactionId=${transactionId}`)
    }
    ...
    return(
        <>
            <AdminHeader breadcrumb="هزینه های جانبی" titleLink={projectId} hrefLink={`/dashboard/projects/${projectId}`} />
            <div className="mt-3 w-[90%] mx-auto grid gap-2">
                {consumptionMaterialInfos.map((item)=>(
                    <ExpensesCard key={item.id} onSelectDelete={handleDelete} onSelectEdit={handleEdit} id={item.id} description={item.description} amount={item.amount} date={item.date} />
                ))}
            </div>
            <Link
                href={`/dashboard/projects/${projectId}/expenses/new&edit`}
